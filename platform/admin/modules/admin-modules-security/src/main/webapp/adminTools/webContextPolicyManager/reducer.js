...
const bins = (state = List(), { type, bin, bins, path, binNumber, pathNumber, value, attribute }) => {
  switch (type) {
    ...
    case 'WCPM_ADD_ATTRIBUTE_MAPPING':
      return state.update(binNumber, (bin) => bin.update('requiredAttributes', (mappings) => mappings.push(fromJS({claim: bin.get('newrequiredClaim'), attribute: bin.get('newrequiredAttribute')}))))
